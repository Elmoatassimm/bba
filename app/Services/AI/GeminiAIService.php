...
{
    /**
     * Valid diagram types that can be generated
     */
    protected const VALID_DIAGRAM_TYPES = [
        'mindmap',
        'flowchart',
        'sequenceDiagram',
        'classDiagram',
     ...
     * Optionally includes a Mermaid diagram in the summary
     *
     ...
     * @return string The summary of the PDF content, optionally with a diagram
     */
    public function summarizePdf(string $filePath, bool $includeDiagram = true, string $diagramType = 'mindmap'): string
    {
        try {
            ...
            // If requested, generate a diagram and add it to the summary
            if ($includeDiagram) {
                try {
                    // Generate the diagram
                    $diagramResult = $this->generateDiagrams($filePath, $diagramType);

                    if (!empty($diagramResult['diagram_code'])) {
                        // Add the diagram to the summary
                        $diagramSection = $this->formatDiagramForSummary($diagramResult, $diagramType);
                        $response = $response . "\n\n" . $diagramSection;
                    }
                } catch (Exception $diagramException) {
                    // Log the error but continue with the summary without the diagram
                    Log::error('Error generating diagram: ' . $diagramException->getMessage());
                    $response = $response . "\n\n*Note: A visual diagram was requested but could not be generated due to an error.*";
                }
            }
     ...
     *
     * @param array $diagramResult The diagram result from generateDiagrams
     * @param string $diagramType The type of diagram
     * @return string The formatted diagram section
     */
    protected function formatDiagramForSummary(array $diagramResult, string $diagramType): string
    {
        $diagramTitle = ucfirst($diagramType) . ' Diagram';
        $explanation = !empty($diagramResult['explanation']) ? $diagramResult['explanation'] : "This diagram visualizes the key concepts from the document.";
        $interpretation = !empty($diagramResult['interpretation']) ? $diagramResult['interpretation'] : "";
        ...
        if (!empty($interpretation)) {
            $formattedSection .= "*How to interpret this diagram:* {$interpretation}\n";
        }
...
PROMPT;
    ...
    public function generateDiagrams(string $filePath, string $diagramType = 'mindmap'): array
    {
        try {
            ...
            if (!in_array($diagramType, self::VALID_DIAGRAM_TYPES)) {
                throw new Exception("Invalid diagram type: {$diagramType}. Valid types are: " . implode(', ', self::VALID_DIAGRAM_TYPES));
            }
            ...
            // If we couldn't extract a valid diagram, try with a more specific prompt
            if (empty($result['diagram_code'])) {
                Log::info('Failed to generate diagram with initial prompt, trying with a more specific prompt');

                $alternativePrompt = $this->createAlternativeDiagramPrompt($text, $diagramType);
                $response = $this->callGeminiApi($alternativePrompt);
                $result = $this->parseDiagramResponse($response, $diagramType);
            }

            if (empty($result['diagram_code'])) {
                throw new Exception("Failed to generate a valid {$diagramType} diagram from the PDF content.");
            }
...
PROMPT;
     ...
     * @return string The prompt
     */
    protected function createDiagramGenerationPrompt(string $text, string $diagramType): string
    {
        $diagramInstructions = $this->getDiagramTypeInstructions($diagramType);

        return <<<PROMPT
You are an expert educator using LearnLM 2.0 Flash Model. Your task is to create a visual representation of academic content that follows learning science principles. You will manage cognitive load by presenting information in a structured, visual format using Mermaid diagrams.

I need you to create a {$diagramType} diagram based on the document provided below. This diagram will be used to help students understand and visualize the content.

Instructions:
1. Analyze the document to identify key concepts, relationships, and structures
2. Create a {$diagramType} diagram using Mermaid syntax that represents the main ideas and their relationships
3. Focus on clarity and educational value - the diagram should help students understand the content
4. Include only the most important information to avoid overwhelming the viewer
5. Use proper Mermaid syntax for {$diagramType} diagrams
{$diagramInstructions}
...
1. A brief explanation of what the diagram represents (2-3 sentences)
2. The complete Mermaid diagram code enclosed in triple backticks with the mermaid tag
...
PROMPT;
     ...
     * @return string The prompt
     */
    protected function createAlternativeDiagramPrompt(string $text, string $diagramType): string
    {
        $diagramInstructions = $this->getDiagramTypeInstructions($diagramType);
        $exampleCode = $this->getExampleDiagramCode($diagramType);

        return <<<PROMPT
You are an expert in creating Mermaid diagrams for educational purposes. Your task is to create a clear, well-structured {$diagramType} diagram based on the document content.

I need you to focus ONLY on creating a valid, syntactically correct Mermaid diagram that represents the key concepts in the document.

Instructions:
1. Focus on the main topics and their relationships in the document
2. Create a simple but effective {$diagramType} diagram using proper Mermaid syntax
3. Ensure the diagram is not too complex - include only 5-10 main elements
4. Follow the exact syntax requirements for Mermaid {$diagramType} diagrams
5. Make sure all nodes and connections are properly defined
{$diagramInstructions}

Here is an example of a valid Mermaid {$diagramType} diagram structure:
...
Provide ONLY the Mermaid diagram code enclosed in triple backticks with the mermaid tag. Ensure it is syntactically correct and properly formatted.
PROMPT;
    }

    /**
     * Get specific instructions for different diagram types
     *
     * @param string $diagramType The type of diagram
     * @return string Instructions specific to the diagram type
     */
    protected function getDiagramTypeInstructions(string $diagramType): string
    {
        switch ($diagramType) {
            case 'mindmap':
                return "6. For mindmaps, use a hierarchical structure with a central concept and related subtopics\n7. Use proper indentation to show relationships between concepts\n8. Keep the mindmap balanced with 3-7 main branches";

            case 'flowchart':
                return "6. For flowcharts, use clear directional flow with proper start and end points\n7. Use appropriate shapes for different types of steps (process, decision, etc.)\n8. Include decision points with yes/no or true/false paths where appropriate";

            case 'sequenceDiagram':
                return "6. For sequence diagrams, identify the key actors/participants\n7. Show the sequence of interactions between participants in chronological order\n8. Include important messages and responses between participants";

            case 'classDiagram':
                return "6. For class diagrams, identify the main classes/entities in the document\n7. Include important attributes and methods for each class\n8. Show relationships between classes (inheritance, association, etc.)";

            case 'erDiagram':
                return "6. For ER diagrams, identify the main entities in the document\n7. Define relationships between entities with proper cardinality\n8. Include key attributes for each entity";

            case 'gantt':
                return "6. For Gantt charts, identify the main tasks or phases from the document\n7. Estimate reasonable durations for each task\n8. Show dependencies between tasks where appropriate";

            case 'pie':
                return "6. For pie charts, identify data that represents parts of a whole\n7. Use 3-7 segments for clarity\n8. Make sure the segments add up to 100% of the whole";

            default:
                return "6. Focus on creating a clear, well-structured diagram\n7. Use appropriate Mermaid syntax for your diagram type\n8. Keep the diagram simple enough to be understood at a glance";
        }
    }

    /**
     * Get example diagram code for different diagram types
     *
     * @param string $diagramType The type of diagram
     * @return string Example code for the diagram type
     */
    protected function getExampleDiagramCode(string $diagramType): string
    {
        switch ($diagramType) {
            case 'mindmap':
                return "mindmap\n  root((Main Topic))\n    Topic 1\n      Subtopic 1.1\n      Subtopic 1.2\n    Topic 2\n      Subtopic 2.1\n      Subtopic 2.2\n    Topic 3\n      Subtopic 3.1";
    ...
    /**
     * Parse the response from Gemini API to extract the Mermaid diagram code
     *
     * @param string $response The response from Gemini API
     * @param string $diagramType The type of diagram that was requested
     * @return array The parsed diagram information
     */
    protected function parseDiagramResponse(string $response, string $diagramType): array
    {
        $result = [
            'diagram_code' => '',
            'explanation' => '',
        ...
        if (preg_match('/```mermaid\s*\n([\s\S]*?)\n```/', $response, $matches)) {
            ...
            // Validate that the diagram code contains the expected diagram type declaration
            $validDiagramStart = false;

            switch ($diagramType) {
                case 'mindmap':
                    $validDiagramStart = strpos($diagramCode, 'mindmap') === 0;
                    break;
                case 'flowchart':
                    $validDiagramStart = strpos($diagramCode, 'flowchart') === 0 || strpos($diagramCode, 'graph') === 0;
                    break;
                case 'sequenceDiagram':
                    $validDiagramStart = strpos($diagramCode, 'sequenceDiagram') === 0;
                    break;
                case 'classDiagram':
                    $validDiagramStart = strpos($diagramCode, 'classDiagram') === 0;
                    break;
                case 'erDiagram':
                    $validDiagramStart = strpos($diagramCode, 'erDiagram') === 0;
                    break;
                case 'gantt':
                    $validDiagramStart = strpos($diagramCode, 'gantt') === 0;
                    break;
                case 'pie':
                    $validDiagramStart = strpos($diagramCode, 'pie') === 0;
                    break;
                default:
                    $validDiagramStart = true; // For unknown types, accept any diagram
            }

            if ($validDiagramStart) {
                $result['diagram_code'] = $diagramCode;
            }
        }

        // Extract explanation (before the diagram)
        $explanationPattern = '/^([\s\S]*?)```mermaid/m';
        if (preg_match($explanationPattern, $response, $matches)) {
            $result['explanation'] = trim($matches[1]);
        }

        // Extract interpretation (after the diagram)
        $interpretationPattern = '/```\s*([\s\S]*?)$/m';
        if (preg_match($interpretationPattern, $response, $matches)) {
            $result['interpretation'] = trim($matches[1]);
        }
